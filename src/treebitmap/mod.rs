    ...
    A: Address,
{
    ...
    /// ```
    pub fn insert(&mut self, ip: A, masklen: u8, value: T) -> Option<T> {
        self.inner
            .insert(&ip.nibbles().as_ref(), masklen as _, value)
    }
    ...
    /// ```
    pub fn remove(&mut self, ip: A, masklen: u8) -> Option<T> {
        self.inner.remove(&ip.nibbles().as_ref(), masklen as _)
    }
    ...
    /// ```
    pub fn exact_match(&self, ip: A, masklen: u8) -> Option<&T> {
        self.inner.exact_match(&ip.nibbles().as_ref(), masklen as _)
    }
    ...
    /// ```
    pub fn exact_match_mut(&mut self, ip: A, masklen: u8) -> Option<&mut T> {
        self.inner
            .exact_match_mut(&ip.nibbles().as_ref(), masklen as _)
    }
    ...
    /// ```
    pub fn longest_match(&self, ip: A) -> Option<(A, u8, &T)> {
        match self.inner.longest_match(&ip.nibbles().as_ref()) {
            Some((bits_matched, value)) => Some((ip.mask(bits_matched), bits_matched as _, value)),
            None => None,
        }
    }
    ...
    /// ```
    pub fn longest_match_mut(&mut self, ip: A) -> Option<(A, u8, &mut T)> {
    ...
    /// ```
    pub fn matches(&self, ip: A) -> impl Iterator<Item = (A, u8, &T)> {
        self.inner
            .matches(ip.nibbles().as_ref())
            .map(move |(bits_matched, value)| (ip.mask(bits_matched), bits_matched as _, value))
    }

    /// Perform match lookup of `ip` and return the all matching
    /// prefixes, designated by ip, masklen, along with its mutable value.
    pub fn matches_mut(&mut self, ip: A) -> impl Iterator<Item = (A, u8, &mut T)> {
        self.inner
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.inner.next() {
            Some((nibbles, masklen, value)) => {
                Some((Address::from_nibbles(&nibbles[..]), masklen as _, value))
            }
            None => None,
        }
    }
